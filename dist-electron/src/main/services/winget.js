...
export class WingetService {
    systemService;
    historyService; // Using any to avoid circularity issues if needed or specific typing
    ...
    async installUpdate(id, onLog) {
        ...
        try {
            ...
            const isInapplicable = error.message.includes('Inapplicable') ||
            ...
            const isTechMismatch = error.exitCode === 2316632107 ||
            ...
            const isHashMismatch = error.exitCode === 2316632081 ||
            ...
            const isFileInUse = error.exitCode === 6 ||
    ...
    parseWingetOutput(output) {
        ...
        if (idStart === -1 || versionStart === -1 || availableStart === -1) {
            ...
            for (const line of dataLines) {
                if (!line.trim() || line.includes('actualizaciones disponibles'))
                    continue;
                updates.push({
