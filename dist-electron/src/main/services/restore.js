...
export class SystemRestoreService {
    ...
    classifyRestoreFailure(output) {
        const normalized = this.normalize(output);
        if (/system protection.*(turned off|disabled)|system restore.*disabled|proteccion del sistema.*desactiv|restauracion del sistema.*desactiv|no hay unidades que tengan habilitada la proteccion/.test(normalized)) {
            return 'system-protection-disabled';
        }
        if (/already been created within the past|restore point.*frequency|ya se ha creado.*punto de restauracion|limite de frecuencia/.test(normalized)) {
            return 'frequency-limit';
        }
        if (/access is denied|permiso denegado|administrator privileges|required elevation|elevacion/.test(normalized)) {
            return 'access-denied';
        }
        if (/vss|volume shadow copy|software shadow copy provider|task scheduler|programador de tareas|rpc server|servicio.*sombra|servicio.*deshabilitad|service.*disabled|service.*not running/.test(normalized)) {
            return 'service-unavailable';
        }
        if (/checkpoint-computer|restore point|punto de restauracion/.test(normalized)) {
            return 'command-failed';
        }
        return 'unknown';
    }
    buildDetails(stdout, stderr, exitCode) {
